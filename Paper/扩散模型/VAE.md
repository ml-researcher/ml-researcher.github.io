...
这里有一个很关键的概念：优化ELBO等价于优化似然！我之前一直觉得优化ELBO是近似优化似然，实际上不是的。根据最大似然估计，我们要选择的是：

$$
\theta^* = \underset{\theta\in \Theta}{\mathrm{argmax~}} \log p(x;\theta)
$$

而如果我们按照ELBO的标准进行选择：

$$
\theta^*, \phi^* = \underset{\theta\in \Theta, \phi\in\Phi}{\mathrm{argmax~}} E_{q(z|x;\phi)}[\log \frac{p(x|z;\theta)p(z)}{q(z|x;\phi)}]
$$
