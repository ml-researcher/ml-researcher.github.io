...
* 首先，我们在训练的时候也不必对每一个可能的x进行建模概率，只需要建模数据集中的点和尽可能多的采样一些非样本点（而非遍历所有可能的非样本点）。我们记这种近似的函数为$\tilde F(\cdot)$，这个函数虽然不能精确把x的概率输出出来，但是样本之间的概率大小关系是差不多的准的，哪些点概率大一些、哪些点概率小一些，有个大致清楚的认知。因此，我们去训练这样一个分类器：将数据集里的图片传进去，输出的概率越大越好，将随机噪声传进去，输出的概率越小越好。这样我们不就训练出来一个$\tilde F(\cdot)$了吗？这样我们就可以用启发式搜索来寻找概率较大的x了。
* 此外，在采样（预测、inference）的时候，也不必遍历寻找精确解，只需要用启发式搜索来找到一个差不多的解。比如NLP里会用beam search，CV里可以对神经网络进行梯度上升（这个时候输入x成了训练的参数）。
